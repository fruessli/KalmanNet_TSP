###############
# SystemModel #
###############
# No idea, what this does so far.

import torch
# Exactly what it sounds like, a "multi-dim" normal.
from torch.distributions.multivariate_normal import MultivariateNormal

# This seems so be the real underlying SystemModel, that the KNet should be able to find.
# Ei, x_t = F * x_t-1 + v_t, y_t = H * x_t + w_t, 
# where v_t ~ N(0,Q), w_t ~ N(0,R), 
# further Q = q^2 * eye, R = r^2 * eye, ny = q^2/r^2
class SystemModel:

    def __init__(self, F, q, H, r, T, T_test, modelname, outlier_p=0,rayleigh_sigma=10000):

        self.modelname = modelname
        # Parameter for the Bernoulli distr
        self.outlier_p = outlier_p
        # Parameter for the Rayleigh distr
        self.rayleigh_sigma = rayleigh_sigma
        ####################
        ### Motion Model ###
        ####################       
        self.F = F
        self.m = self.F.size()[0]

        # Noise covariance in State
        self.q = q
        self.Q = q * q * torch.eye(self.m)

        #########################
        ### Observation Model ###
        #########################
        self.H = H
        self.n = self.H.size()[0]

        # Noise covariance in Observation
        self.r = r
        self.R = r * r * torch.eye(self.n)

        # Assign T and T_test
        # T is Trajectory length according to the paper
        # Not used in SysMdl itself
        self.T = T
        self.T_test = T_test

    #####################
    ### Init Sequence ###
    #####################
    # Gives first 1st and 2nd moments of the state. 
    # It has to be known where to begin the Sequence.
    def InitSequence(self, m1x_0, m2x_0):

        self.m1x_0 = m1x_0
        self.m2x_0 = m2x_0

    #########################
    ### Update Covariance ###
    #########################
    # Allows for changes in the Noise distr.
    # Never used.
    def UpdateCovariance_Gain(self, q, r):

        self.q = q
        self.Q = q * q * torch.eye(self.m)

        self.r = r
        self.R = r * r * torch.eye(self.n)

    def UpdateCovariance_Matrix(self, Q, R):

        self.Q = Q

        self.R = R

    #########################
    ### Generate Sequence ###
    #########################
    # Generate a Sequence of states and their observations.
    def GenerateSequence(self, Q_gen, R_gen, T):
        # Pre allocate an array for current state
        # up to t = T
        self.x = torch.empty(size=[self.m, T])
        # Pre allocate an array for current observation
        # up to t = T
        self.y = torch.empty(size=[self.n, T])
        # Set x0 to be x previous
        self.x_prev = self.m1x_0
        
        # Outliers
        # Each element of b_matrix is 1 with prob outlier_p,
        # and 0 otherwise.
        if self.outlier_p > 0:
            b_matrix = torch.bernoulli(self.outlier_p *torch.ones(T))

        # Generate Sequence Iteratively
        for t in range(0, T):
            ########################
            #### State Evolution ###
            ########################            
            # Process Noise
            # x_t = F * x_t-1 + v_t in 3 steps:
            # 1) x_t = F * x_t-1
            # 2) Draw eq from N(0,Q_gen)
            # 3) x_t = x_t + eq
            if self.q == 0:
                xt = self.F.matmul(self.x_prev)            
            else:
                xt = self.F.matmul(self.x_prev)
                mean = torch.zeros([self.m])              
                distrib = MultivariateNormal(loc=mean, covariance_matrix=Q_gen)
                eq = distrib.rsample()
                # eq = torch.normal(mean, self.q)
                eq = torch.reshape(eq[:],[self.m,1])
                # Additive Process Noise
                xt = torch.add(xt,eq)

            ################
            ### Emission ###
            ################
            # Observation Noise
            # y_t = H * x_t + w_t in 3 steps:
            # 1) y_t = H * x_t
            # 2) Draw er from N(0,R_gen)
            # 3) y_t = y_t + er
            if self.r == 0:
                yt = self.H.matmul(xt)           
            else:
                yt = self.H.matmul(xt)
                mean = torch.zeros([self.n])            
                distrib = MultivariateNormal(loc=mean, covariance_matrix=R_gen)
                er = distrib.rsample()
                er = torch.reshape(er[:],[self.n,1])
                # mean = torch.zeros([self.n,1])
                # er = torch.normal(mean, self.r)
                
                # Additive Observation Noise
                yt = torch.add(yt,er)
            
            # Outliers
            # Maybe this has smth to do with the Rayleigh distr?
            # btdt = sigma * sqrt(-2*log(rand))
            # y_t + btdt
            if self.outlier_p > 0:
                if b_matrix[t] != 0:
                    btdt = self.rayleigh_sigma*torch.sqrt(-2*torch.log(torch.rand(self.n,1)))
                    yt = torch.add(yt,btdt)

            


            ########################
            ### Squeeze to Array ###
            ########################

            # Save Current State to Trajectory Array
            self.x[:, t] = torch.squeeze(xt)

            # Save Current Observation to Trajectory Array
            self.y[:, t] = torch.squeeze(yt)

            ################################
            ### Save Current to Previous ###
            ################################
            self.x_prev = xt

    ######################
    ### Generate Batch ###
    ######################
    # Generate 'size' many Sequences.
    # The Input is the y of these Sequences, since they are given.
    # The Target is the x of these Sequences, since the NN estimates them.
    def GenerateBatch(self, size, T):

        # Allocate Empty Array for Input
        self.Input = torch.empty(size, self.n, T)

        # Allocate Empty Array for Target
        self.Target = torch.empty(size, self.m, T)

        ### Generate Examples
        for i in range(0, size):
            # Generate Sequence

            self.GenerateSequence(self.Q, self.R, T)

            # Training sequence input
            self.Input[i, :, :] = self.y

            # Training sequence output
            self.Target[i, :, :] = self.x